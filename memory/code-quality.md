...
- **Block-buffered stdout** — default stdout buffer type switched from line-buffered to block-buffered. `printf` output may be lost on process exit if the buffer is not flushed. For child processes (test scripts via `/bin/sh "$t"`), this resolves naturally: fork+exec creates a separate process that flushes stdio on normal exit. For the `/init` process itself, `reboot -f` is a direct syscall that never returns, so atexit flush never happens — any buffered `echo` output (including `TEST_DONE`) is silently dropped. Additionally, buffered parent-echo output bursts onto the UART when the 8 KB buffer fills, appearing out-of-order relative to child test output. **Fix: write all protocol markers in `/init` directly to `/dev/console` (`printf 'MARKER\n' > /dev/console`), bypassing the block buffer.** The `/dev/console` redirect opens the device, does a single write() syscall, and closes — immediate and ordered. Applied to: `BOOT_OK`, `> TEST RUN:`, `< TEST PASS:`, `< TEST FAIL:`, summary line, and `TEST_DONE` in `lib/initramfs.sh`.
...
- **`if out=$(cmd); then`** or **`out=$(cmd) || fallback`** → Toybox sh bug: variable assignment always exits 0; command's real exit code is swallowed; `if` branch always evaluates true, `||` branch never fires. Fix: `cmd > /tmp/out.txt 2>/dev/null`; test file with `[ -s /tmp/out.txt ]`; read for diagnostics. Detected by `test-static-analysis.sh` check 1.
