...
- **Phase 6a (branch feat/hw-bootstrap, pending PR):** `lib/hw-bootstrap.sh` sets up hardware board testing infra; `lib/board.sh` `board_reset()` pulses USB relay; verified on real hardware (Lenovo/Arch, VisionFive 2). Stack: DHCP via systemd-networkd built-in DHCPServer (`[DHCPServer]` in 50-vf2.network, options 66+67 for U-Boot PXE); TFTP via `atftpd` (atftp package). Key quirks: Arch uses `uucp` serial group (not `dialout`); NM must be marked unmanaged for `HW_IFACE`; `ConfigureWithoutCarrier=yes` goes in `[Network]` not `[Link]`; relay may be CP210x (`10c4:ea60`) not CH340 (`1a86:7523`) — set in `local.mk`; atftpd requires `--daemon --no-fork --user user.group` (Arch has no `nogroup`, uses `nobody`); dnsmasq rejected because Landlock sandbox blocks all non-default lease paths
