...
- **dev-test (PR #56, merged 2026-08-16):** `make dev-test` — ≤5-min branch verification gate covering ≥50% of 35 decision paths. Fixed core (lint + C build + 4 CI tests + tinyconfig/defconfig/localconfig VM smokes) always runs; random draw (uniform LCG, SEED=N reproducible, uniform probability — budget_ok() governs time) covers remaining paths. Environment-aware: board paths D4–D6 skipped with notice when HAS_BOARD=no. `make hook-dev-test` toggles opt-in pre-push block. Coverage map: `tests/ci/coverage-map.md`. LCG bug: `r=$(fn)` runs in subshell — always inline LCG arithmetic, never call via `$()`.
