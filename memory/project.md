...
- **dev-test (PR #56, open):** `make dev-test` — ≤5-min branch verification gate covering ≥50% of 35 decision paths. Fixed core (lint + C build + 4 CI tests + tinyconfig/defconfig/localconfig VM smokes) always runs; random draw (uniform LCG, SEED=N reproducible, uniform probability — budget_ok() governs time) covers remaining paths. Environment-aware: board paths D4–D6 skipped with notice when HAS_BOARD=no. `make hook-dev-test` toggles opt-in pre-push block. Coverage map: `tests/ci/coverage-map.md`. LCG bug: `r=$(fn)` runs in subshell — always inline LCG arithmetic, never call via `$()`. snapshot.c: CLOCKSOURCE/ASLR/DMESG_RESTRICT/KPTR_RESTRICT emit `n/a` when sysfs/proc path absent (fixes 480_snapshot on tinynsconfig). perf-event.c: `sched_yield()` before `read()` fixes TASK_CLOCK returning 0 on `CONFIG_HIGH_RES_TIMERS=n`+`HZ=250` (scheduler only calls `update_curr()` on ticks; 100k-iter loop finishes in <1 tick; caught by dev-test on stable/tinynsconfig). Verified: 5/5 `make dev-test` PASS (68–74% coverage per run).
