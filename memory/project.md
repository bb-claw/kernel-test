...
- **dev-test (feat/dev-test, in progress):** `make dev-test` — ≤5-min branch verification gate covering 50% of 35 identified decision paths. Fixed core (lint + C build + 4 CI tests + tinyconfig/defconfig/localconfig VM smokes) covers top 25%; random draw (SEED=N, reproducible) covers next 25%. Environment-aware: skips board/TCG paths with notice on laptop. `make hook-dev-test` installs opt-in pre-push integration. Coverage map: `tests/ci/coverage-map.md`. Files: `scripts/dev-test.sh`.
