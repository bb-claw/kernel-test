...
verify_build() {
    ...
    if [[ "$result" == VERIFIED* ]]; then
        ...
        obj="drivers/$SUBSYSTEM/$(basename "${cfile%.c}.o")"
        {
            ...
            printf '# COMPILE_TEST bypasses arch-specific '\''depends on'\'' guards.\n\n'
            printf 'make ARCH=x86_64 tinyconfig\n'
            printf 'scripts/config --enable CONFIG_COMPILE_TEST\n'
            printf 'scripts/config --enable CONFIG_%s\n' "$sym"
            printf 'make ARCH=x86_64 olddefconfig\n'
            printf 'make ARCH=x86_64 %s\n\n' "$obj"
