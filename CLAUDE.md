...
- `presets/<dir>.mk` — committed preset auto-included by the Makefile based on `$(notdir $(CURDIR))`; `presets/kernel-test-stable.mk` sets `STABLE_RELEASE ?= 7.1`; `presets/kernel-test-stable-rc.mk` sets `KERNEL_TREE`, `LABEL`, `GCC`, `BUILD_TIMEOUT`, `STABLE_RC_BRANCH`; `presets/kernel-test-next.mk` sets `KERNEL_TREE`, `LABEL`, `LINUX_NEXT := 1`; all three set `DATA_REPO ?= $(HOME)/git/kernel-test-data`; mainline (`kernel-test`) uses `presets/kernel-test-mainline.mk` which sets only `DATA_REPO`
