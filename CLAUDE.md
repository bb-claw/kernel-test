...
- **Kernel configs:** `defconfig`, `tinyconfig`, `allnoconfig`, `allmodconfig`, `randconfig`, `rand500config`, `randdefconfig`; plus hardware-specific `laptopconfig` and `localconfig` (not in default `CONFIGS`)
  - Bootable (build + VM test): `defconfig`, `tinyconfig`, `allnoconfig`, `rand500config`, `randdefconfig`, `laptopconfig`, `localconfig`
