...
if ! command -v musl-gcc &>/dev/null || ! command -v musl-clang &>/dev/null; then
    ...
    if make -C "$AT_DIR" clean all ARCHES="x86_64" 2>"$BUILD_STDERR"; then
...
begin_test "at-i386-build"
if command -v gcc &>/dev/null && gcc -m32 -x c -o /dev/null - </dev/null 2>/dev/null; then
    tmpdir; I386_STDERR="$_LAST_TMPDIR/at-i386-stderr.txt"
    if make -C "$AT_DIR" "bin/i386/arena-test" 2>"$I386_STDERR"; then
        assert_file_exists "$AT_DIR/bin/i386/arena-test" "i386 binary built"
    else
