...
if ! command -v musl-gcc &>/dev/null; then
    printf '  skip  musl-gcc not installed — skipping build tests\n'
    printf '        (install: sudo pacman -S musl  or  sudo apt-get install musl-tools)\n'
elif ! command -v musl-clang &>/dev/null; then
    printf '  skip  musl-clang not installed — skipping build tests\n'
    printf '        (musl-clang is provided by musl-tools on Debian; on Arch: musl + clang)\n'
else
    if make -C "$SC_DIR" clean all 2>/tmp/sc-build-stderr.txt; then
...
PAYLOAD="$(python3 -c 'import sys; sys.stdout.buffer.write(bytes(range(256)))' 2>/dev/null \
    || printf '%256s' '' | tr ' ' 'x')"

"$SC_BIN" "$SC_RX" 115200 "$LOGFILE" &
SC_PID=$!

# Wait for the binary to open the pty and create the logfile.
count=0
while [[ ! -e "$LOGFILE" ]]; do
    sleep 0.05
    (( count++ )) || true
    if [[ $count -ge 40 ]]; then
        ...
        stop_pty_pair
        fail "serial-capture did not create logfile within 2s"
        break 2
    fi
done

# Write known payload to the tx side.
printf '%s' "$PAYLOAD" > "$SC_TX"

# Give the capture loop time to drain the pty buffer.
sleep 0.2
...
CAPTURED="$(cat "$LOGFILE" 2>/dev/null || true)"
if [[ "$CAPTURED" == "$PAYLOAD" ]]; then
    pass "data integrity: captured bytes match payload"
else
    fail "data integrity: payload mismatch (got $(printf '%s' "$CAPTURED" | wc -c) bytes, want $(printf '%s' "$PAYLOAD" | wc -c))"
fi
...
# Wait for the binary to start and block in read().
count=0
while [[ ! -e "$LOGFILE2" ]]; do
    sleep 0.05
    (( count++ )) || true
    if [[ $count -ge 40 ]]; then
        kill "$SC_PID" 2>/dev/null || true
        stop_pty_pair
        fail "serial-capture did not start within 2s"
        break 2
    fi
done
...
if ! "$SC_BIN" /dev/null abc /dev/null 2>/tmp/sc-baud-stderr.txt; then
    ERRMSG="$(cat /tmp/sc-baud-stderr.txt)"
...
if ! "$SC_BIN" /dev/null 99 /dev/null 2>/tmp/sc-baud-stderr.txt; then
    ERRMSG="$(cat /tmp/sc-baud-stderr.txt)"
