...
static int open_serial(const char *device, int baud)
{
   ...
   * misbehave when opened read-only. */
	...
	/* (tcflag_t) casts: termios constants are int; c_cflag is tcflag_t
   * (unsigned). */
...
int main(int argc, char *argv[])
{
	...
	if (errno != 0 || end == argv[2] || *end != '\0' || baud_l <= 0 ||
	    baud_l > INT_MAX) {
	...
	/* SA_RESTART intentionally absent: SIGTERM/SIGINT interrupts read() with
   * EINTR. */
