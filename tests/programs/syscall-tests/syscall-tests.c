...
static int test_32bit(void) {
  ...
  if (fd < 0) {
    ...
    if (got < 0) {
      skip(
          "lseek64 to 5 GiB failed — filesystem may not support large offsets");
    } else if (got != target) {
  ...
  void *p = mmap(NULL, sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
                 -1, 0);
  if (p == MAP_FAILED) {
...
static int test_io_uring(void) {
  ...
  /* Map SQ ring */
  size_t sq_sz =
      (size_t)p.sq_off.array + (size_t)p.sq_entries * sizeof(uint32_t);
...
static int test_fds(void) {
  ...
  if (efd < 0) {
    ...
    if (write(efd, &wval, sizeof(wval)) != (ssize_t)sizeof(wval)) {
      ...
      uint64_t rval = 0;
      if (read(efd, &rval, sizeof(rval)) != (ssize_t)sizeof(rval) ||
          rval != 7) {
...
static int test_landlock(void) {
  ...
  long rfd =
      syscall(SYS_landlock_create_ruleset, &rattr, sizeof(rattr), (unsigned)0);
  if (rfd < 0) {
