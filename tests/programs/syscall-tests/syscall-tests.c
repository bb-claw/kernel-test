...
/* io_uring — same musl-gcc header availability issue; stable ABI since
 * Linux 5.1 */
...
static int test_io_uring(void)
{
	...
	/* Map SQ ring */
	size_t sq_sz = (size_t)p.sq_off.array +
	...
	long ret = syscall(SYS_io_uring_enter, ring_fd, (unsigned)1,
			   (unsigned)1, (unsigned)IORING_ENTER_GETEVENTS,
			   (void *)NULL, (size_t)0);
	if (ret < 0) {
...
static int test_fds(void)
{
	...
	if (tfd < 0) {
		...
		if (timerfd_settime(tfd, 0, &ts, NULL) < 0) {
			/* EINVAL/ENOSYS: timer clock unavailable on this config (e.g. 32-bit
       * allnoconfig) */
...
static int test_unix(void)
{
     ...
     * EAFNOSUPPORT/EPROTONOSUPPORT: CONFIG_UNIX=n (sockets exist, AF_UNIX not
     * registered) */
...
static int test_landlock(void)
{
	...
	long rfd = syscall(SYS_landlock_create_ruleset, &rattr, sizeof(rattr),
			   (unsigned)0);
	if (rfd < 0) {
	...
	long ar = syscall(SYS_landlock_add_rule, ruleset_fd,
			  (unsigned)LANDLOCK_RULE_PATH_BENEATH, &pattr,
			  (unsigned)0);
	close(tmp_fd);
	if (ar < 0) {
...
static int test_bpf(void)
{
   ...
   * Exercises bpf() syscall, BPF verifier, and JIT/interpreter path. */
	struct st_bpf_insn prog[] = {
		{ (uint8_t)(ST_BPF_ALU64 | ST_BPF_MOV | ST_BPF_K), 0, 0,
		  0 }, /* r0 = 0 */
	...
	long fd = syscall(SYS_bpf, (int)ST_BPF_PROG_LOAD, &attr,
			  (unsigned int)sizeof(attr));
	if (fd < 0) {
		int e = errno;
		if (e == ENOSYS) {
			skip("CONFIG_BPF_SYSCALL not available");
			return 0;
		}
		if (e == EPERM) {
			skip("bpf() EPERM — unprivileged BPF restricted");
			return 0;
		}
...
static int test_sysvipc_shm(void)
{
	...
	if (id < 0) {
		int e = errno;
		if (e == ENOSYS) {
			skip("CONFIG_SYSVIPC not available");
			return 0;
		}
...
static int test_sysvipc_sem(void)
{
	...
	if (semop(id, &v_op, 1) < 0) {
       ...
       * CONFIG_COMPAT_32BIT_TIME (depends on CONFIG_POSIX_TIMERS). */
			skip("semop ENOSYS: CONFIG_POSIX_TIMERS absent (i386 musl/semtimedop "
			     "routing)");
...
{
	if (argc != 2) {
		fprintf(stderr, "usage: syscall-tests"
				" <32bit|seccomp|io_uring|fds|unix|landlock"
				"|bpf|sysvipc-shm|sysvipc-sem|sysvipc-msg>\n");
		return 1;
	}
	const char *cmd = argv[1];
	if (strcmp(cmd, "32bit") == 0)
		return test_32bit();
	if (strcmp(cmd, "seccomp") == 0)
		return test_seccomp();
	if (strcmp(cmd, "io_uring") == 0)
		return test_io_uring();
	if (strcmp(cmd, "fds") == 0)
		return test_fds();
	if (strcmp(cmd, "unix") == 0)
		return test_unix();
	if (strcmp(cmd, "landlock") == 0)
		return test_landlock();
	if (strcmp(cmd, "bpf") == 0)
		return test_bpf();
	if (strcmp(cmd, "sysvipc-shm") == 0)
		return test_sysvipc_shm();
	if (strcmp(cmd, "sysvipc-sem") == 0)
		return test_sysvipc_sem();
	if (strcmp(cmd, "sysvipc-msg") == 0)
		return test_sysvipc_msg();
