...
typedef struct {
	ArenaInfo info; /* the general information of one arena and its blocks without
                     the necessarity to process this on demand */
...
static void *arena_alloc(Arena *a, size_t n)
{
   ...
   */
	while (a->head->used + aligned > a->head->size) {
		if (a->head->next) {
			a->head =
				a->head->next; /* reuse block from a previous cycle */
		} else {
			size_t sz = aligned > a->info.arena_block_size ?
					    aligned :
					    a->info.arena_block_size;
...
static int test_reset(int *cycles_out)
{
	...
	for (int i = 0; i < CYCLES; i++) {
		...
		if (block_count(&a) != blocks_expected) {
			ok = 0;
			break;
		}
	}
...
static int test_alignment(int *align_bytes_out)
{
	...
	*align_bytes_out = (int)align;
	...
	static const size_t odd_sizes[] = { 1,	3,  5,	7,  9, 15,
					    17, 31, 33, 63, 65 };
	int ok = 1;
...
static int test_page_size(int *page_size_out)
{
	...
	*page_size_out = ps;
	...
	if (ps < 4096)
		return 0; /* sanity: a page must be at least 4 KiB */
...
static int test_stress(int *blocks_out, int *readback_ok_out)
{
	...
	const size_t ITERS =
		(STRESS_MB * MB) / CHUNK_SZ; /* 8192 chunks total   */
   ...
   */
	unsigned char **ptrs =
		(unsigned char **)malloc(ITERS * sizeof(unsigned char *));
	if (!ptrs) {
		*blocks_out = 0;
		*readback_ok_out = 0;
		return 0;
...
static int test_mmap(int *pages_out)
{
	...
	unsigned char *mem =
		(unsigned char *)mmap(NULL, map_size, PROT_READ | PROT_WRITE,
